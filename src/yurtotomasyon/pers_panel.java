...
public class pers_panel extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					pers_panel frame = new pers_panel();
	...
	public pers_panel() {
		...
		JButton btndemeler = new JButton("Kay\u0131tlar");
		...
		JButton btnikayetBildir = new JButton("\u015Eikayetler");
		...
		JLabel lblKalanIzin = new JLabel("\u0130zinli \u00D6\u011Frenciler");
		lblKalanIzin.setFont(new Font("Ubuntu", Font.BOLD, 16));
		lblKalanIzin.setBounds(12, 12, 151, 15);
		...
		JLabel lblBorlar = new JLabel("Toplam \u00D6\u011Frenciler");
		lblBorlar.setFont(new Font("Ubuntu", Font.BOLD, 16));
		lblBorlar.setBounds(12, 12, 171, 15);
		...
		pers_profil panels = new pers_profil();
		...
		try {
			...
			PreparedStatement sorgu2 = con.prepareStatement("Select count(ogr_id) as sayi from ogrenciler");
			PreparedStatement sorgu3 = con.prepareStatement("Select count(izin_id) as en from izinler where izin_tarihi like '%"+df.format(simdikiZaman)+"%'");
			...
			lblNewLabel_1.setIcon(format);
			
			
			}
			while(sonuc2.next()) {
				lblTl.setText(sonuc2.getString("sayi"));
				
				}
			while(sonuc3.next()) {
				label_2.setText(sonuc3.getString("en"));
		...
		pers_izin izinpage = new pers_izin();
		pers_duyuru duyurupage = new pers_duyuru();
		pers_odeme odemepage = new pers_odeme();
		pers_mesaj mesajpage = new pers_mesaj();
		pers_etkinlik etkinlik = new pers_etkinlik();
		pers_profil pers_profil = new pers_profil();
